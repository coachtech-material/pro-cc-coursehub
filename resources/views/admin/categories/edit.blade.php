...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('admin.categories.index') }}" class="inline-flex items-center text-sm text-gray-500 hover:text-indigo-600 transition-colors">
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">カテゴリ編集</h1>
        <p class="mt-1 text-sm text-gray-500">{{ $category->name }}</p>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" x-data="{ submitting: false }" @submit="submitting = true">
            @csrf
            @method('PUT')
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.categories.index') }}" class="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium rounded-lg px-4 py-2.5 transition-all duration-150">キャンセル</a>
            </div>
        </form>
    </div>
</div>
