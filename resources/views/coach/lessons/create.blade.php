...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">レッスン作成</h1>
        <p class="mt-1 text-sm text-gray-500">{{ $chapter->title }}</p>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form method="POST" action="{{ route('coach.courses.chapters.lessons.store', [$course, $chapter]) }}" x-data="{ submitting: false }" @submit="submitting = true">
            @csrf
            ...
            <div class="mb-5">
                ...
                <textarea name="body" id="body" rows="10" required
                    class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 px-3 py-2.5 border font-mono text-sm">{{ old('body') }}</textarea>
            </div>
