...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">チャプター編集</h1>
        <p class="mt-1 text-sm text-gray-500">{{ $chapter->title }}</p>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form method="POST" action="{{ route('coach.courses.chapters.update', [$course, $chapter]) }}" x-data="{ submitting: false }" @submit="submitting = true">
            @csrf
            @method('PUT')
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('coach.courses.chapters.index', $course) }}" class="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium rounded-lg px-4 py-2.5 transition-all duration-150">キャンセル</a>
            </div>
        </form>
    </div>
</div>
